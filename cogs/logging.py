...
class LoggerCog(commands.Cog):
    ...
    async def on_message_delete(self, message):
        if message.author.bot:
            return
        text = f"🗑 {message.author} (ID: {message.author.id}) удалил сообщение: {message.content}"
        await self.log(text)
    ...
    async def on_message_edit(self, before, after):
        ...
        text = (f"✏️ {before.author} (ID: {before.author.id}) отредактировал сообщение в {before.channel.mention}:\n"
                f"До:\n" f"> {before.content}" + f"\nПосле:\n" f"> {after.content}")
        await self.log(text)

    @commands.Cog.listener()
    async def on_member_join(self, member):
        text = f"🤝 {member} (ID: {member.id}) присоединился к серверу"
        await self.log(text)

    @commands.Cog.listener()
    async def on_member_remove(self, member):
        text = f"👋 {member} (ID: {member.id}) покинул сервер"
        await self.log(text)
    ...
    async def on_voice_state_update(self, member, before, after):
        ...
        if mute_changed:
            if before.self_mute != after.self_mute:
                ...
                changes_list.append("администратор отключил микрофон" if after.mute else "админ включил микрофон")
